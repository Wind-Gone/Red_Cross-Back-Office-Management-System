...
<script>
        ...
        columns: [{
            ...
            formatter: function (value, row, index) {
                ...
                var watchStr = '<button type="button"\n' +
                    ...
                    '                                                                    onclick="location.href=\'${pageContext.request.contextPath}/fi/find?id=' + aId + '\'">详情\n' +
                    '                                                            </button>'
                var delStr = '<button type="button"\n' +
                    '                                                                    class="btn bg-olive btn-xs"\n' +
                    '                                                                    onclick="location.href=\'${pageContext.request.contextPath}/fi/delete?id=' + aId + '\'">删除\n' +
                    '                                                            </button>'
                var updateStr = '<button type="button"\n' +
                    '                                                                    class="btn bg-olive btn-xs"\n' +
                    '                                                                    onclick="location.href=\'${pageContext.request.contextPath}/fi/find?id=' + aId + '\'">更新\n' +
                    '                                                            </button>'
    ...
    function searchID() {
        ...
        if (IDend == null || IDend == "")
            IDend = 129000;
        $('#mytab').bootstrapTable('filterBy', {fi_id: [IDbegin, IDend]},
            {
                'filterAlgorithm': function (row, filters) {
                    return row.fi_id >= filters.fi_id[0] && row.fi_id <= filters.fi_id[1];
                }
            }
        );
    }
    ...
    function searchAll() {
        ...
        $('#mytab').bootstrapTable('filterBy', {fi_id: [IDbegin, IDend], fi_date: [DATEbegin, DATEend]},
            {
                'filterAlgorithm': function (row, filters) {
                    if (filters.fi_date[0] == "")
                        if (row.fi_id >= filters.fi_id[0] && row.fi_id <= filters.fi_id[1])
                            return true;
                    if (filters.fi_id[0] == "") {

                        if (row.fi_date >= filters.fi_date[0] && row.fi_date <= filters.fi_date[1])
                            return true;
                    }
                    if (row.fi_date >= filters.fi_date[0] && row.fi_date <= filters.fi_date[1] && row.fi_id >= filters.fi_id[0] && row.fi_id <= filters.fi_id[1])
                        return true;
                    return false;
                }
            }
    ...
    function searchDATE() {
        ...
        if (DATEend == null || DATEend == "")
            DATEend = '2020-06-31';
        $('#mytab').bootstrapTable('filterBy', {fi_date: [DATEbegin, DATEend]},
            {
                'filterAlgorithm': function (row, filters) {
                    return row.fi_date >= filters.fi_date[0] && row.fi_date <= filters.fi_date[1];
                }
            }
        );
    }
