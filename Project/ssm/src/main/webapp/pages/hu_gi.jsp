...
<script>
    $('#mytab').bootstrapTable({
        ...
        columns: [{
            ...
            formatter: function (value, row, index) {
                ...
                var watchStr = '<button type="button"\n' +
                    ...
                    '                                                                    onclick="location.href=\'${pageContext.request.contextPath}/gi/find?id=' + aId + '\'">详情\n' +
                    '                                                            </button>'
                var delStr = '<button type="button"\n' +
                    '                                                                    class="btn bg-olive btn-xs"\n' +
                    '                                                                    onclick="location.href=\'${pageContext.request.contextPath}/gi/delete?id=' + aId + '\'">删除\n' +
                    '                                                            </button>'
                var updateStr = '<button type="button"\n' +
                    '                                                                    class="btn bg-olive btn-xs"\n' +
                    '                                                                    onclick="location.href=\'${pageContext.request.contextPath}/gi/find?id=' + aId + '\'">更新\n' +
                    '                                                            </button>'
        ...
        function searchID() {
            ...
            if (IDend == null || IDend == "")
                IDend = 210000;
            $('#mytab').bootstrapTable('filterBy', {gi_id: [IDbegin, IDend]},
                {
                    'filterAlgorithm': function (row, filters) {
                        return row.gi_id >= filters.gi_id[0] && row.gi_id <= filters.gi_id[1];
                    }
                }
            );
        }
    ...
    function searchAll() {
        ...
        $('#mytab').bootstrapTable('filterBy', {gi_id: [IDbegin, IDend], gi_date: [DATEbegin, DATEend]},
            {
                'filterAlgorithm': function (row, filters) {
                    if (filters.gi_date[0] == "")
                        if (row.gi_id >= filters.gi_id[0] && row.gi_id <= filters.gi_id[1])
                            return true;
                    if (filters.gi_id[0] == "") {

                        if (row.gi_date >= filters.gi_date[0] && row.gi_date <= filters.gi_date[1])
                            return true;
                    }
                    if (row.gi_date >= filters.gi_date[0] && row.gi_date <= filters.gi_date[1] && row.gi_id >= filters.gi_id[0] && row.gi_id <= filters.gi_id[1])
                        return true;
                    return false;
                }
            }
    ...
    function searchDATE() {
        ...
        if (DATEend == null || DATEend == "")
            DATEend = '2020-06-31';
        $('#mytab').bootstrapTable('filterBy', {gi_date: [DATEbegin, DATEend]},
            {
                'filterAlgorithm': function (row, filters) {
                    return row.gi_date >= filters.gi_date[0] && row.gi_date <= filters.gi_date[1];
                }
            }
        );
    }
